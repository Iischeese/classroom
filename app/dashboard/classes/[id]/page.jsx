...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${data.grade_level}${
      data.grade_level > 2 ? "th" : data.grade_level > 1 ? "nd" : "st"
    } Grade ${data.name}`,
  };
}
...
export default async function Page({ params }) {
  ...
  if (classroom.message)
    return (
      <Error
        ...
        desc="The classroom you are looking for could have been moved, or deleted"
      />
    );
  ...
  if (auth == 0)
    return (
      <Error
        title="No access."
        desc="You do not have access to this classroom. If you believe this is a mistake contact your teacher or school head."
      />
    );
        ...
        <div className="absolute w-full h-full top-0 left-0 z-30 flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2 absolute bottom-0 m-3">
            ...
            <SubTitle>
              ...
              {classroom.grade_level > 2
                ? "th"
                : classroom.grade_level > 1
                ? "nd"
                : "st"}{" "}
              grade | {user.prefix} {user.last_name}
            </SubTitle>
          </div>
          {
            // Edit Mode
            signedIn.user_id == classroom.user_id ? (
              <div className="absolute right-0 top-0 m-3 flex gap-2">
                <Button link={`/dashboard/classes/${params.id}/edit`}>
                  Settings
                </Button>
